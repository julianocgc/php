...
if(isset($_COOKIE["valorFrete"]) && $_COOKIE["valorFrete"]>0 && isset($_COOKIE["carrinhoAtual"]))
{
	...
	$data = date("Y-m-d");
	$res = mysql_query("INSERT INTO pedido(email,data,valorTotal,precoFrete) VALUES('$email','$data',0,'$frete')");
	$idPedido = mysql_insert_id ($con);	// obtém o id gerado para o pedido (campo auto-incremento)
	...
	{
		$res = mysql_query("SELECT preco,precoPromocao FROM produto WHERE idProduto=$id");
		...
		$res = mysql_query("INSERT INTO itemPedido VALUES($idPedido,$id,$preco,$quant)");
	}
	$res = mysql_query("UPDATE pedido SET valorTotal=$valorTotal WHERE idPedido=$idPedido");
